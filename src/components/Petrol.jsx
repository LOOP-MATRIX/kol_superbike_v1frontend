  ...
  const fetchPetrolRecords = async (purchaseId) => {
    try {
      ...
      if (data.success && data.data.length > 0) {
        ...
        setPetrolRecords([]); // No data or 404
      }
    } catch (error) {
    ...
    <div className="p-6">
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md mb-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input name="petrolCost" value={formData.petrolCost} onChange={handleChange} className="p-2 border rounded" placeholder="Petrol Cost" type="number" />
